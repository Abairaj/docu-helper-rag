...
async def index_documents_async(documents : List[Document], batch_size : int = 50):
    ...
    log_info(f"vectorstore indexing preparing to add {len(documents)} documents into vectorstore",Colors.DARKCYAN)
    
    batches = [documents[i:i+batch_size] for i in range(0,len(documents),batch_size)]
    log_info(f"Documents is split into {len(batches)} batches of {len(batches)} documents each")
...
async def main():
    ...
    log_info("TavilyCrawl: Starting to Crawl documentations from https://python.langchain.com/",Colors.PURPLE)
    ...
    log_success(f"Texsplitter created {len(splitted_documents)} chunks from {len(all_docs)} documents.")
